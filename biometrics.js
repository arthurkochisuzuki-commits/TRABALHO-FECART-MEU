...
class BiometricsEngine {
  ...
  verifyFacialTopology(data, width, height, box) {
    const { x, y, w, h } = box;
    if (w < 24 || h < 28) return false;

    // 1. Proporção facial
    const ratio = h / w;
    if (ratio < 1.0 || ratio > 1.95) return false;

    // 2. Extrair luminância Y em 3 faixas horizontais do candidato:
    // Faixa 1: Testa (12% a 30% da altura)
    // Faixa 2: Olhos (32% a 54% da altura)
    // Faixa 3: Bochechas/Nariz (56% a 78% da altura)
    ...
    for (let py = yStart; py < yEnd; py++) {
      ...
      for (let px = xStart; px < xEnd; px++) {
        ...
        if (relY >= 0.12 && relY <= 0.30) {
          foreheadSum += lum;
          foreheadCount++;
        } else if (relY > 0.32 && relY <= 0.54) {
          eyeSum += lum;
          eyeCount++;
    ...
    if (foreheadCount < 20 || eyeCount < 25 || cheekCount < 25) return false;

    const avgForehead = foreheadSum / foreheadCount;
    const avgEye = eyeSum / eyeCount;
    const avgCheek = cheekSum / cheekCount;

    // Em um rosto humano, a cavidade ocular é mais escura que a testa ou as bochechas.
    // Em uma mão ou objeto, a luminância é uniforme ou os dedos criam faixas verticais contrárias.
    const isEyeDarkerThanForehead = (avgForehead - avgEye) > 1.5 || (avgEye / (avgForehead + 0.001) < 0.98);
    const isEyeDarkerThanCheek = (avgCheek - avgEye) > 1.0 || (avgEye / (avgCheek + 0.001) < 0.985);

    if (!isEyeDarkerThanForehead && !isEyeDarkerThanCheek) {
      // Falha: não possui a assinatura óptica de uma cavidade ocular (ex: mão, braço ou objeto)
      return false;
    }

    // Validação de simetria bilateral entre os dois olhos
    if (leftEyeCount > 8 && rightEyeCount > 8 && noseBridgeCount > 6) {
      const avgLeftEye = leftEyeSum / leftEyeCount;
      const avgRightEye = rightEyeSum / rightEyeCount;

      // Os olhos não podem ter divergência luminosa extrema (> 45%)
      const eyeDisparity = Math.abs(avgLeftEye - avgRightEye) / (Math.max(avgLeftEye, avgRightEye) + 0.001);
      if (eyeDisparity > 0.45) {
        return false;
      }
    }
  ...
  /**
   * Detector Unificado de Rosto Humano
   * Segmentação de pele YCbCr + Validação Topológica Anatômica
   * Compartilhado entre a detecção do feed ao vivo e o cadastro fotográfico
   */
  findFaceBoundingBox(data, width, height) {
    ...
    for (let y = 10; y < height - 10; y += step) {
      for (let x = 10; x < width - 10; x += step) {
        ...
        const isSkin = (cb >= 77 && cb <= 127 && cr >= 133 && cr <= 173 && yVal >= 38 && r > g && r > b);
    ...
    const minSkinThreshold = Math.max(150, (width * height) / 120);
    if (skinPixels < minSkinThreshold || totalWeight === 0) {
      return null;
    }

    const rawW = (maxX - minX);
    const rawH = (maxY - minY);
    const candidateBox = { x: minX, y: minY, w: rawW, h: rawH };

    // Verifica a assinatura anatômica do rosto para descartar mãos ou objetos
    ...
    const boxW = Math.max(28, Math.min(width * 0.85, rawW * 1.15));
    const boxH = Math.max(34, Math.min(height * 0.90, rawH * 1.25));
